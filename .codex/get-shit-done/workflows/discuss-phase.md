...
```bash
INIT=$(node "/home/ubuntu/workspace/ros2_ws/.codex/get-shit-done/bin/gsd-tools.cjs" init phase-op "${PHASE}")
...
**Batch mode support:** Parse optional `--batch` from `{{GSD_ARGS}}`.
...
```bash
node "/home/ubuntu/workspace/ros2_ws/.codex/get-shit-done/bin/gsd-tools.cjs" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
node "/home/ubuntu/workspace/ros2_ws/.codex/get-shit-done/bin/gsd-tools.cjs" commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
