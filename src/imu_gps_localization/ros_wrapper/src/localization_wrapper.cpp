...
LocalizationWrapper::LocalizationWrapper(ros::NodeHandle &nh) {
  ...
  // Subscribe topics.
  imu_sub_ =
      nh.subscribe("/imu/data", 10, &LocalizationWrapper::ImuCallback, this);
  gps_position_sub_ =
      nh.subscribe("/fix", 10, &LocalizationWrapper::GpsPositionCallback, this);
...
void LocalizationWrapper::GpsPositionCallback(
  ...
  gps_data_ptr->lla << gps_msg_ptr->latitude, gps_msg_ptr->longitude,
...
void LocalizationWrapper::LogGps(
  ...
  file_gps_ << std::fixed << std::setprecision(15) << gps_data->timestamp << ","
